...
This library does not have any functionality for (or opinion about) implementing joueur authentication and retrieving JWTs to begin with. Those details will vary depending on your setup, but in most cases, you will use a regular HTTP request to authenticate your users and then save their JWTs in local storage or in a cookie if successful.
...
2. `tokenNotExpired` - allows you to check whether there is a non-expired JWT in local storage. This can be used for conditionally showing/hiding elements and stopping navigation to certain routes if the joueur isn't authenticated
...
* Add support for **[linking different joueur accounts](https://docs.auth0.com/link-accounts)** with the same joueur.
* Support for generating signed [Json Web Tokens](https://docs.auth0.com/jwt) to call your APIs and **flow the joueur identity** securely.
* Analytics of how, when and where users are logging in.
* Pull data from other sources and add it to the joueur profile, through [JavaScript rules](https://docs.auth0.com/rules).
